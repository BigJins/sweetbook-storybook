...
- 반복 보일러플레이트를 줄이기 위해 Lombok을 적극 활용한다.
- plan에 명시되지 않은 generic public setter 추가 금지.
- 상태 변경과 값 수정은 `markFailed()`, `retry()`, `updateBodyText()`처럼 의도가 드러나는 메서드를 우선한다.
- setter가 정말 필요해 보이면 임의로 추가하지 말고 plan 근거를 먼저 확인한다.

## 모델 선택 원칙

작업 성격에 맞춰 사용 모델만 바꾼다. 문서에 모델명을 박지 말고 원칙으로 둔다 — 모델 라인업은 시간이 지나며 변하므로.

- **상위 모델 (예: Opus 등급)**: 복잡한 리서치, 설계·계획 수립, 머지 리뷰, 통합 결정, 막힌 문제의 재해석.
- **빠른 모델 (예: Sonnet 등급)**: 명세 기반 구현, 반복 수정, 보일러플레이트, 테스트 작성.
- **승격 규칙**: 빠른 모델로 진행 중 두 번 이상 막히거나 plan과 충돌이 보이면 상위 모델로 전환.
- **강등 규칙**: 상위 모델로 결정·계획이 끝나면 다음 단계는 빠른 모델로 내려 비용·속도 확보.

실제 적용 방법은 도구별로 다름 — Claude 앱·Code에선 새 세션 시작 시 모델 선택만 바꾸면 됨. 본 저장소의 dispatch 프롬프트(`docs/agents/*.md`)에는 권장 모델 등급만 한 줄로 명시.
